...
from urbanev_forecast import conditional_od as core
...
def train_one(kind,seed,x32,residual,a,budget,curves,receipt):
    ...
    save(0)
    curves.append({'model':kind,'seed':seed,'epoch':0,'lr':0.,'training_mse':float(np.mean(residual.astype(np.float64)**2)),'optimizer_steps':0,'selection_rmse':None})
    for epoch in range(1,41):
        ...
        for start in range(0,len(order),4096):
            ...
            steps+=1;receipt['optimizer_steps']=receipt.get('optimizer_steps',0)+1;total+=float(loss.detach())*len(ix)
        curves.append({'model':kind,'seed':seed,'epoch':epoch,'lr':lr,'training_mse':total/len(x32),'optimizer_steps':steps,'selection_rmse':None})
...
def execute(a,c,receipt):
    ...
    q0=base.ridge_predict(z,anchor,beta);residual=(target-q0).astype(np.float32);x32=z.astype(np.float32);del z,target,q0
    ...
    curve_fields=['model','seed','epoch','lr','training_mse','optimizer_steps','selection_rmse'];csvout(a.public_output/'training_curves.csv',curves,curve_fields)
    ...
    csvout(a.public_output/'selection_scores.csv',selection_rows,['epoch']+score_fields);csvout(a.public_output/'training_curves.csv',curves,curve_fields)
    ...
    for item in selection:
        ...
        if item['selected']['epoch']==0:f=np.zeros_like(qbase);cross=np.zeros_like(qbase) if kind=='OD_PRODUCT' else None
        else:
            net=core.InteractionNet(kind,seed).cuda(0);net.load_state_dict(torch.load(path,map_location='cuda:0',weights_only=True));f,cross,elapsed=infer(net,x32,kind=='OD_PRODUCT');inference_seconds+=elapsed;del net
    ...
    for seed in core.SEEDS:
        main=predictions['OD_PRODUCT',str(seed)]
        for reference in ('BASE_OD','OD_SEPARABLE','OD_CONCAT_MLP','OO_PRODUCT','NATIVE'):
            for h in ((3,12) if reference=='NATIVE' else base.HORIZONS):
                ...
                for scope in ('terminal_H','path_1_to_H'):
                    ...
                    for post in ('raw','clip_0_1'):
                        ...
                        report,g1,g2=core.grouped_losses(yy,rb,mt,groups);tags={'seed':seed,'candidate':'OD_PRODUCT','reference':reference,'horizon':h,'target_scope':scope,'postprocess':post};loss_reports.append({**tags,**report})
                        ar=next(r for r in scores if r['model']=='OD_PRODUCT' and r['seed']==str(seed) and r['horizon']==h and r['target_scope']==scope and r['postprocess']==post)
    ...
    for kind in core.MODELS:
        for support in ('FOUR_H','H3_H12_COMMON'):
            for post in ('raw','clip_0_1'):
                for scope in ('terminal_H','path_1_to_H'):
                    rr=[r for r in summaries if r['model']==kind and r['support']==support and r['postprocess']==post and r['target_scope']==scope];row={'model':kind,'support':support,'postprocess':post,'target_scope':scope,'seed_count':2}
                    for metric in ('rmse','mae'):
                        vv=[r['macro_'+metric] for r in rr];row.update({metric+'_mean':float(np.mean(vv)),metric+'_min':min(vv),metric+'_max':max(vv)})
                    means.append(row)
    dump(a.public_output/'model_summary.json',{'per_run':summaries,'seed_metric_means':means,'training_runs':runs,'uncertainty':'two seeds and one previously exposed DEV window; descriptive only','primary':'OD_PRODUCT; raw four-H endpoint RMSE','scientific_automatic_admission':False,'sota_claim':False})
    ...
    if receipt['optimizer_steps']!=18880 or len(scores)!=160:raise ValueError('Incomplete formal execution')
    receipt.update(status='CONDITIONAL_OD_INTERACTION_COMPLETE_REVIEW_REQUIRED',fit_rows=240900,fit_origins=876,select_origins=14,dev_origins=14,selection_score_rows=len(selection_rows),valid_dev_score_rows=152,missing_native_rows=8,loss_comparison_cells=len(loss_reports),paired_origin_rows=len(origin_rows),cache_decodes=4,inference_seconds_synchronized=inference_seconds,neural_training_seconds_synchronized=sum(r['training_seconds_synchronized'] for r in runs),gpu_peak_allocated_bytes=max(r['gpu_peak_allocated_bytes'] for r in runs),gpu_peak_reserved_bytes=max(r['gpu_peak_reserved_bytes'] for r in runs),scientific_automatic_admission=False,sota_claim=False)
...
def main():
    ...
    if c['protocol_id']!='CONDITIONAL_OD_INTERACTION_V1_20260913' or c['limits']!=base.LIMITS or c['models']!=list(core.MODELS):raise ValueError('Protocol mismatch')
    ...
    except Exception as e:receipt.update(status='CONDITIONAL_OD_INTERACTION_BLOCKED',error_type=type(e).__name__,error=str(e));raise
    finally:
