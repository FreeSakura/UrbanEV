...
当前优先维护持续事件完整稿、几何/优化实现和冻结结果复现。历史充电预测与审计模块仍可维护。提交应说明具体问题、改动后的行为及验证范围。

## 开发环境

按[快速开始](docs/guides/quickstart.md)创建虚拟环境。当前科学检查安装 `.[test,evidence]`；构建Word另加 `.[manuscript]`、Pandoc与Poppler。只有维护历史预测模型时才需要 CPU PyTorch 与 `.[test,research]`。
...
| 当前Word论文 | 重建受影响图表与Word，保持正文/附录/冻结证据一致，检查版面与原生公式 |
| 历史LaTeX论文 | 构建受影响版本并核对原论文清单 |
...
| 持续事件代码 | 运行对应事件测试；不启动历史预测smoke或真实数据拟合 |
| 当前论文源码 | 按 `paper/persistent_events/README.md` 构建；临时输出与已交付工件分开 |
| 历史论文源码 | 构建受影响的LaTeX版本并核对原论文清单 |
...
CI按改动范围运行：普通导航只做资料检查；事件实现运行CPU科学测试；当前稿件/图表走独立Word构建；预测模块才触发PyTorch合成smoke；历史论文才触发LaTeX。修改依赖或CI本身仍保守检查全部路线。失效链接、遗漏证据和错误汇总由资料检查发现；`scripts/repository.py` 不触发实验。
