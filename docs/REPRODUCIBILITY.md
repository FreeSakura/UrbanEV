...
For licensed-data recomputation, place the UrbanEV `occupancy.csv` and `inf.csv` files under the registered data root. For Paris development, pass `--development-shard` with a CSV containing `date`, `Station`, `Available`, `Charging`, `Passive`, and `Other`. The default search accepts only `development_state_shard.csv`; a complete `train.csv` is streamed only with `--allow-full-source`. The tool rejects development shards or package requests beyond 30 November 2020, rebuilds the non-available-port fraction, aligns target timestamps and string-valued entity IDs, validates duplicates and denominators, casts to the recorded dtype, and requires the exact stored target hash before scoring.
