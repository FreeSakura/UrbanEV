...
原研究登记、冻结六折的执行依赖、模型源码、配置、公开数值及执行回执保持原身份。通用开发 CLI 的代码锁和输出目录检查另行精简，见[代码审阅](DEFENSIVE_CODE_REVIEW.md)。论文 PDF 与其来源未因资料整理而改写。绘图/报告渲染脚本更新了文档输出位置；历史 source manifest 仍表示原运行版本，不应被改成新路径下的当前哈希。
