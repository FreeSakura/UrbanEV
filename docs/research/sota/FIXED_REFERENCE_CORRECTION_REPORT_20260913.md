...
LINEAR_REPAIR相对固定底座的DEV RMSE降低1.183273%、MAE降低3.319957%；机制窗分别降低2.260664%/3.850659%。它仍弱于完整ridge：机制窗RMSE/MAE高0.147747%/0.550492%，DEV高0.618746%/0.354097%。完整ridge的优势不能单独归因于多出的45个原点，还包含不同联合估计方式。因其使用同一参考残差和纠错训练行，本轮不能笼统解释为“没有任何可利用纠错信号”。也不能把这个普通线性更新收益当作交互贡献或SOTA。
...
神经行对三个种子及四H等权平均；A/B/G是单点损失的平均，不是宏RMSE平方差。主方法在机制窗的三种子平均RMSE/MAE相对固定底座降低1.211987%/1.929497%。三个种子分别均好于固定底座，但均弱于线性纠错及完整ridge；分离与拼接网络的整体机制窗成绩也更好，不能把改善认定为专用乘积结构的价值。

主方法12个种子×H单元的A均为正，G有10正、2负；负值都在H6（种子15/17），说明这些固定方向有正对齐，但幅度代价抵消了它。没有据此求新alpha或修改输出。

机制窗中，主方法平均A比线性纠错低约2.77%，修正能量B高约16.15%，平均端点MSE收益少约43.17%。线性相对主方法的G差可分为2(A_L−A_P)=0.000037515087301与B_P−B_L=0.000148882386343。这是锁定预测之间的代数分解，不能认作因果归因或直接移植为DEV成因。
...
主方法行是三种子指标均值。相对底座的结论必须注明支持：H3/H12 raw的主方法平均MAE比固定底座低0.095485%，但RMSE高1.192848%；不能将四H双指标恶化推广为所有口径。native有不同预训练和信息范围，只在原调用H3/H12作参考，未补H6/H9推理。对native的raw RMSE/MAE分别高3.357793%/18.679630%，匹配clip则高3.234406%/18.630730%。本轮没有SOTA证据，完整ridge的旧成绩是重建而非刷新。
...
消除运行时底座更换后，收益仍未跨时段保持；这限制了“只要解决迁移就能成功”的解释，却没有定位唯一原因。时间变化、表示及估计/优化误差仍需区分。唯一留待下一轮人工讨论的问题是：普通线性更新已能利用的关系之外，是否还有跨时段保持的非线性交互，还是网络主要用不稳定修正替换了可用线性关系？这不是新增残差计算、模型或切分授权。本轮不依据机制诊断重新缩放、不补训练或更换底座。
...
研究侧已依据回传完成数值、理论与解释复核，没有独立访问私有数组或重新执行训练。执行侧完成263项测试和隐私核验；这些一致性核验不等于独立全流程复现。
