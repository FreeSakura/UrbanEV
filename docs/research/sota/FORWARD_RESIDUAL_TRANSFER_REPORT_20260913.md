...
本轮结果不追溯改变[SOTA距离快照](SOTA_GAP_ASSESSMENT_20260913.md)中的前一轮成绩，也不覆盖完整官方六折或最新方法。RMSE主张不要求MAE必须非劣，但必须有明确可比的领先证据；这里尚未建立。
...
完整、已执行的负结果有助于停止沿同一方案继续试参数，却不是SOTA突破。下一问题需要重新判断纠错机制与剩余可预测误差的关系；本轮不追加非选中checkpoint诊断、新损失或新数据读取，交回用户人工审核。
