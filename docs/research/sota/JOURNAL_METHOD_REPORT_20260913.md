...
本方案随后完成唯一一次探针，结果通过研究侧限定复核，详见[执行与复核报告](DUAL_RISK_PROBE_REPORT_20260913.md)。共同下降条件可等价写为均值方向与多数误差符号一致；原子估计并未在四个单元上变得更准确，不能据此主张新的原子机制。负导数支持当前样本上小步长改善的局部存在性，实际可选步长、≥1%收益及独立外推仍未验证。当前不自动进入下一协议。
