...
日期：2026-09-13。状态：**文献驱动的理论与冻结探针方案；不是SOTA成果声明。** 复杂理论与实验规格由研究协作侧提出，执行侧核对来源、整理公式与实现；本节保留真实数据运行前的方案，实际结果另见[执行报告](../calibration/DUAL_RISK_PROBE_REPORT_20260913.md)。
...
完整问题→假设→机制→反证的阅读记录见[期刊阅读笔记](../../theory/JOURNAL_READING_NOTES_20260913.md)，包含8篇重点参考、1篇勘误和2篇直接竞争工作。研究侧另补充以下5篇，执行侧均完成Crossref身份核验，阅读范围逐项注明；这些身份核验不等于全文实验复现。
...
本方案随后完成唯一一次探针，结果通过研究侧限定复核，详见[执行与复核报告](../calibration/DUAL_RISK_PROBE_REPORT_20260913.md)。共同下降条件可等价写为均值方向与多数误差符号一致；原子估计并未在四个单元上变得更准确，不能据此主张新的原子机制。负导数支持当前样本上小步长改善的局部存在性，实际可选步长、≥1%收益及独立外推仍未验证。当前不自动进入下一协议。
