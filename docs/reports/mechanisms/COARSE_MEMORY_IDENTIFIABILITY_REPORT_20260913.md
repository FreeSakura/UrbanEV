...
这回应了物理学、经济学与交通调度三条线的共同问题：观测到的占用量如何由进入、驻留、退出和容量约束形成。严谨推导、八篇研究侧选读与交通补充见[完整理论报告](../../theory/COARSE_MEMORY_THEORY_REPORT_20260913.md)，来源核查与更多直接先例见[阅读笔记](../../theory/PHYSICS_ECONOMICS_TRAFFIC_READING_20260913.md)。
...
HMM一致性是已知生成参数下，同一条件概率的两条计算路径一致；不是新模型战胜或打平经过训练的经典基线。研究侧复核接受本轮公式、实现与限定结论，未要求重跑；该复核没有再次执行有限求和或本地测试。
...
本次结束于合成机制检查，保留全部旧NO_GO和已消费claim。没有读取尾部、第三折验证/测试或Paris数据；定时任务继续PAUSED，未使用额度重置卡。每轮完成后返回人工审核，不启动定时任务或自动下一轮。下一真实阶段需要另立完整且可证伪的协议，不能由本次结果自动触发。
