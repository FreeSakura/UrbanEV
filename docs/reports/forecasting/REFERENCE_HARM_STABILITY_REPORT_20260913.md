...
**实验完整执行，但整体稳定化主张未成立。** 主方法较原PRODUCT_MSE略有四H平均误差改善，也减少了增损；然而四H宏指标仍未超过ridge底座，成绩的跨种子离散度反而增加，通用MLP从相同惩罚获得的平均收益更大。本轮不以此否定全部交互方法，也不改选其他方案作为事后主方法。H3/H12共同支持的结论另列，不能混写。
...
六个神经方案的四H平均RMSE和MAE均高于共同底座。原PRODUCT_MSE在这批新种子上，相对BASE的平均RMSE/MAE分别高0.161539%/0.970183%；上一轮平均底座增益没有在本批种子中保持。这加强了初始化及选择敏感性的疑问，不追溯改写旧结果。
...
对固定种子修正a_s，MSE(q_s)=MSE(q⁰)−2E[(Y−q⁰)a_s]+E[a_s²]。即使修正彼此更接近，它们与底座误差的对齐程度也可能不同，因此更低预测分歧与更高成绩标准差可以同时发生。这只是误差恒等式，不是已定位的不稳定成因。
...
POSREG相对MSE的增损下降25.7818%、收益下降21.9934%、平均修正幅度下降26.0239%。增损减少0.002171873846，同时丢失收益0.001706586903，净MAE改善0.000465286943。增损/平均修正幅度约从0.472686变为0.474233，没有降低；这个派生比率只用于解释，未参与选择，不是新评分门。最终DEV输出减少增损成立，精准识别有害修正尚未成立；L1并非与POSREG等效，也不能反称收益完全来自统一缩放。
...
这里评估的是训练目标及各自预注册checkpoint选择的组合，部分配对epoch不同；不是固定权重或固定训练步下孤立替换损失的因果效应。本轮没有追加未注册checkpoint评分。
...
H3/H12共同支持raw端点上，POSREG相对BASE的RMSE/MAE分别低0.170532%/0.100105%；相对PRODUCT_MSE却是RMSE高0.058865%、MAE低0.324599%。四H与共同支持不是同一对象，不能将四H结论推广到H3/H12。匹配clip后相对native的RMSE低0.170366%，MAE仍高13.782424%。
...
这些结果不证明时长、非线性交互或所有参考风险方法无效，也不把普通MIX/L1事后升级为新主方法。唯一留待下一轮人工讨论的问题是：可学习交互是否主要依赖底座的样本内残差结构，而非能在训练内前向样本外残差重复出现的预测信号？这只是替代解释，不是本轮已证明的原因；本轮不授权交叉拟合、新数据读取或加大惩罚强度。
...
研究侧已依据回传完成数值、理论与解释复核，未独立访问私有数组或重跑全流程。源码、测试及隐私核验由执行侧完成；数值验收不能替代独立复现。
